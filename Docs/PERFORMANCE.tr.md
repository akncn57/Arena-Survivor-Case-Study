...
ağırlık, tek materyal, 1024 x 512 ASTC atlas). Ayrıntılar `TECH.tr.md` > Optimize asset'ler. Başka hiçbir şey değişmedi.
...
zamanlı gölge yerine blob shadow kullanıyor. Ayrıntılar `TECH.tr.md` > Render ayarları.
...
rifle dokuları 2048 -> 512. Ayrıntılar `TECH.tr.md` > Optimize asset'ler > Oyuncu / Rifle.
...
ayrışma, yarıçap 1,2 m). Ayrıntılar `TECH.tr.md` > Düşmanlar > Ayrışma.
...
- **Titreme düzeltmesi.** Telefonda benchmark sırasında oyuncunun etrafında biriken düşmanların titrediği fark edildi.
  ...
  değiştiriyordu. Ayrışma sertliği 1 -> 0,5 ve 1/60 sn'lik alt adımlar ile düzeldi (ayrıntılar `TECH.tr.md` > Ayrışma).
...
Değişiklik (ayrıntılar `TECH.tr.md` > Animasyon optimizasyonu):
...
edildi; oyun kamerasından daha yakın bir açıdan bile ayırt edilemediği için en ucuzu seçildi. Ayrıntılar `TECH.tr.md` >
Düşman materyali.
